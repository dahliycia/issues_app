...
const NEW_ISSUE_REQUIRED_FIELDS = ['title', 'description', 'submitter']
...
const issuesListHandler = async (request, h) => { 
  ...
  const data = issuesList.slice(offset, offset+limit)
...
const issuesAddHandler = async (request, h) => {
  ...
  for (const field of NEW_ISSUE_REQUIRED_FIELDS) {
    if (data[field] === undefined) return Boom.badRequest(`The body must contain: "${field}"`)
  }
...
const routes = [
  ...
  },
  {
    ...
    config: {
      ...
      cors: true,
      description: 'Add a new issue. Payload should have a form of: {title: <string>, description: <string>, submitter: <string>}'
    }
  },
  {
    method: 'POST',
    ...
    config: {
      ...
      cors: true,
      description: 'Change state of an existing issue. Payload should have a form of: {state: <string: pending/closed>, comment: {author: <string>, text: <string>}'
    }
  }
];
